...
python3 whisper_online.py --model large-v3 ../data/speeches/Justice.mp3 --language en --min-chunk-size 1 > out.txt
```
...
```
# online server
python3 whisper_online_server.py --host 0.0.0.0 --port 43007 --model large-v3 --warmup-file ./data/jfk.wav --language zh --min-chunk-size 1 > out.txt
```

Client example:

```
arecord -f S16_LE -c1 -r 16000 -t raw -D default | nc 35.92.84.228 43007
```
...
In mac
1. First, install `sox` using Homebrew (if you don't have Homebrew, install it first from https://brew.sh/):
...
```bash
rec -q -t raw -r 16000 -b 16 -c 1 -e signed -L - | nc 35.92.84.228 43007
